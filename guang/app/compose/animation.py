...
class ExtremeValue:
    ...
    def get_max(self, **x):
        ...
        for key in x:
            ...
            self.x_max[key] = max(self.x_max[key], x[key])
...
def lorenz_Traces(x0=1, y0=3, z0=5, lambd = 0.01,
                  k=3, t_start=0, t_end=10, all_step=900):

    t = np.linspace(t_start, t_end, all_step)
    Traces = []
    for i in range(k):
        trace = Lorenz.Trace(t=t, xyz=[x0+lambd*np.random.rand(),
    ...
    def get_frames(self, total_frames = 100, period=1):
        ...
        for i in range(total_frames):
            ...
            frames.append(go.Frame(data=data,
                                   traces=[i for i in range(self.k)]  ####THIS IS THE LINE THAT MUST BE INSERTED
                                   ) )
...
class Draw2D:
    ...
    def get_layout(self, width=980,height=880, duration=10,
        ...
        layout = go.Layout(width=width, height=height,
                           ...
                           updatemenus=[dict(type="buttons",
                                             buttons=[dict(label="Play",
                                                           method="animate",
                                                           args=[None,
                                                                 dict(frame=dict(duration=duration, redraw=redraw),
